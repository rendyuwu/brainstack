...
export function DiscoverClient({ articles, topicFilters, totalCount }: DiscoverClientProps) {
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '32px 32px 80px' }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--tx-2)', margin: 0 }}>
            {totalCount} articles across {topicFilters.length - 1} topic stacks
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
          ...
          >
            {(['list', 'grid'] as const).map((mode) => {
              const iconName = mode === 'list' ? 'list' : 'layers';
              return (
                <button
                  key={mode}
